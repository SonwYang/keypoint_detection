...
class Cutout:
    def __init__(self, num_holes=20, max_h_size=7, max_w_size=7, fill_value=0, prob=0.9):
...
class RandomAugment(object):
    ...
    def get_random_ops(self):
        ...
        return [(op, .5, self.M) for op in sampled_ops]
...
if __name__ == '__main__':
    ...
    # a(img)
    imgPath = glob.glob('train_images/*.npy')[0]
    img = np.load(imgPath)
    img2 = a(img)
    plt.subplot(121)
    plt.imshow(img)
