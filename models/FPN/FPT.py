...
from models.FPN.self_trans import SelfTrans
from models.FPN.rendering_trans import RenderTrans
from models.FPN.grounding_trans import GroundTrans
# import nn as mynn
# from dropblock import DropBlock2D


class FPT(nn.Module):
    def __init__(self, feature_dim, planes, with_norm='none', upsample_method='bilinear'):
        ...
        if with_norm != 'none':
            self.fpn_p5_1x1 = nn.Sequential(*[nn.Conv2d(planes[-1], feature_dim, 1, bias=False), norm(feature_dim)])
            self.fpn_p4_1x1 = nn.Sequential(*[nn.Conv2d(planes[-2], feature_dim, 1, bias=False), norm(feature_dim)])
            self.fpn_p3_1x1 = nn.Sequential(*[nn.Conv2d(planes[-3], feature_dim, 1, bias=False), norm(feature_dim)])
            self.fpn_p2_1x1 = nn.Sequential(*[nn.Conv2d(planes[-4], feature_dim, 1, bias=False), norm(feature_dim)])
            ...
            self.fpn_p5_1x1 = nn.Conv2d(planes[-1], feature_dim, 1)
            self.fpn_p4_1x1 = nn.Conv2d(planes[-2], feature_dim, 1)
            self.fpn_p3_1x1 = nn.Conv2d(planes[-3], feature_dim, 1)
            self.fpn_p2_1x1 = nn.Conv2d(planes[-4], feature_dim, 1)
