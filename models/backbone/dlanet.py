...
def dla60(pretrained=None, **kwargs):  # DLA-60
    ...
    model = DLA([1, 1, 1, 2, 3, 1],
                ...
                block=Bottleneck, return_levels=True, **kwargs)
    model.out_shapes = [1024, 512, 256, 128, 32, 16]
    if pretrained is not None:
...
def dla102(pretrained=None, **kwargs):  # DLA-102
    ...
    model = DLA([1, 1, 1, 3, 4, 1], [16, 32, 128, 256, 512, 1024],
                block=Bottleneck, residual_root=True, return_levels=False, **kwargs)
    if pretrained:
        model.load_pretrained_model(data='imagenet', name='dla102')
    return model


def dla102x(pretrained='imagenet', **kwargs):  # DLA-X-102
    BottleneckX.expansion = 2
    model = DLA([1, 1, 1, 3, 4, 1], [16, 32, 128, 256, 512, 1024],
                block=BottleneckX, residual_root=True, return_levels=True, **kwargs)
    model.out_shapes = [1024, 512, 256, 128, 32, 16]
    if pretrained is not None:
        model.load_pretrained_model(pretrained, 'dla102x', hash='ad62be81')
    return model
