...
from models.backbone.hrnet.module_helper import ModuleHelper
from models.backbone.hrnet.logger import Logger as Log
...
def HRNetBackbone(arch, bn_type='torchbn'):
        from models.backbone.hrnet.hrnet_config import MODEL_CONFIGS

        if arch == 'hrnet18':
            arch_net = HighResolutionNet(MODEL_CONFIGS[arch],
                                         bn_type=bn_type,
                                         bn_momentum=0.1)
            arch_net.out_ch = MODEL_CONFIGS[arch]['STAGE4']['NUM_CHANNELS']
...
if __name__ == '__main__':
    backbone = HRNetBackbone('hrnet48')
