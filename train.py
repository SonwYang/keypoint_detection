...
class TrainModule(object):
    def __init__(self, dataset, num_classes, model, decoder, down_ratio):
        ...
        self.dataset_phase = {'dota': ['train', 'valid'],
                              'hrsc': ['train', 'test']}
    ...
    def train_network(self, args):
        ...
        dsets_loader['train'] = torch.utils.data.DataLoader(dsets['train'],
                                                           ...
                                                           drop_last=True,
                                                           collate_fn=collater)
        dsets_loader['valid'] = torch.utils.data.DataLoader(dsets['valid'],
                                                           batch_size=args.batch_size,
                                                           shuffle=True,
                                                           num_workers=args.num_workers,
                                                           pin_memory=True,
        ...
        for epoch in range(start_epoch, args.num_epoch+1):
            ...
            np.savetxt(os.path.join(save_path, 'valid_loss.txt'), valid_loss, fmt='%.6f')
